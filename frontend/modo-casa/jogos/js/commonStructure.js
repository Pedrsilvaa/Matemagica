...
export function Cronometro(userSession) {
  ...
  function countTimer() {
    ...
    if (
      cronometro[1].toString().length == 1 &&
      cronometro[0].toString().length == 1
    ) {
...
export function stage4(userSession, bncc, fase) {
  document.querySelector("html").innerHTML = `<head>
    ...
    <link
      rel="icon"
      type="image/x-icon"
      href="/frontend/images/logo menor.png"
    />
  ...
  <body>
    <main>
      <div class="md1">
        ...
        <h4 class="h4-1">
          Parabéns, você avançou para o nível <span id="next-level">2</span>
        </h4>
        <h4>
          Cada desafio é uma poção mágica para aprender e se divertir!<br />
        </h4>
        <h4>Tempo: <span id="tempo">0:00</span></h4>
        <h4>Taxa de acertos: <span id="acertos">0%</span></h4>
        <i>
          Habilidade trabalhada da BNCC:
          <span id="hab"> EF03MA01</span>
        </i>
        <button id="finalbt" class="btn-purple">continuar</button>
        <img
          src="../../../images/porquinho-da-india 1.png"
          alt="Hamster"
          class="hamster"
        />
      </div>
